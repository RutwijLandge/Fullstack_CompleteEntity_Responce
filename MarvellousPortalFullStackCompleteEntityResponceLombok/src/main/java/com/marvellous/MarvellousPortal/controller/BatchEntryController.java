...
public class BatchEntryController
{
    ...
    @GetMapping
    public ResponseEntity<?> getAll()       //datatype not known now
    {
        List <BatchEntry> allData = batchEntryService.getAll();

        if((allData != null) && (!allData.isEmpty()))
        {
            return new ResponseEntity< >(allData,HttpStatus.OK);  //interview Q
        }
        else
        {
    ...
    @PostMapping
    public ResponseEntity<BatchEntry> creatEntry(@RequestBody BatchEntry myentry)
    {
        try
        {
